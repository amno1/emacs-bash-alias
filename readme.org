...
Enable the use of Bash aliases in built-in shell-command and async-shell-comand.
...
Both functions install themselves as redirects for built-in ones, so if you use keyboard shortcuts to call shell-command, the new ones will automatically be called.


If you change your Bash aliases, you will have to regenerate a new alias table. You can do it manually for the session and future sessions by running:

    M-x bash-alias-collect-aliases RET

* How it works

Provided shell-command-with-aliases and async-shell-command-with-aliases wrap built-in ones but do a lookup on alias before they actually execute shell-command. There are other ways to do this, advice or a pre-command-hook, but I think a simple wrapper is the safest alternative in this case. If someone can convince me that advice would be a better option, I am not impossible to change that one.

The core of this is collecting aliases from a bash as Emacs sees it. Since Bash aliases can be scattered in different files and sourced via .bashrc, it is not possible to collect them by just parsing a file or two. The only "failsafe" way is to actually ask Bash which aliases does it know about.
