...
Enable the use of Bash aliases in GNU Emacs.
...
When bash-alias-mode is enabled, you can use explicitly bash-alias-shell-command and bash-alias-async-shell-command in places where you would use shell-command and async-shell-command:

    M-x bash-alias-shell-command or M-x bash-alias-async-shell-command

Both functions install themselves as shortcut redirects. Shell-commans is bound by default to M-! and async-shell-command to M-&. Whichever keys they are bound to will be automatically rebound to bash-alias commands.
...
Aliases are generated each time the mode is started.
